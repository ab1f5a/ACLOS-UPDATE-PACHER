...
DEPLOY_DIR = os.path.join(os.environ['APPDATA'], "ACLOS Tools")
        ...
        if res.status_code == 200:
            ...
            if remote_v != CURRENT_VERSION:
                ...
                for i, line in enumerate(log_lines):
                    if i == 0:
                        print_bordered_line(f"更新日志: {line}", C)
                    else:
                        print_bordered_line(f"          {line}", C)
...
def deploy_patcher():
    ...
    try:
        ...
        shortcut = shell.CreateShortCut(os.path.join(desktop, "ACLOS2.13.1.lnk"))
        shortcut.TargetPath = dst
        shortcut.WorkingDirectory = DEPLOY_DIR
        shortcut.save()
